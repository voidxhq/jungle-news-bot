...
def find_clean_image(keywords):
    """Tries to find a clean, real photo based on keywords"""
    print(f"🔍 Searching for a clean image for: {keywords}...")
    try:
        # We search Unsplash (Clean, high-res, no watermarks)
        search_url = f"https://source.unsplash.com/featured/1600x900/?{keywords.replace(' ', ',')}"
        response = requests.get(search_url, allow_redirects=True, timeout=5)
        
        if response.status_code == 200:
            return response.url  # This is the direct link to a beautiful, clean photo
    except:
        return None
    return None
...
if clean_image:
    ...
    print("✅ Found a clean, unique image!")
else:
    # Fallback to the original (watermarked) image only if search fails
    cover_image = scraper.top_image 
    print("⚠️ No clean image found. Using original (may have watermark).")
